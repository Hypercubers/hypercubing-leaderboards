...
pub async fn block_logins(ctx: PoiseCtx<'_>) -> AppResult {
    ...
    ctx.reply("Logins are now blocked. Use `/unblock` to unblock all actions.")
        .await?;
    Ok(())
}
...
pub async fn block_submissions(ctx: PoiseCtx<'_>) -> AppResult {
    ...
    ctx.reply("New submissions are now blocked. Use `/unblock` to unblock all actions.")
        .await?;
    Ok(())
}
...
pub async fn block_user_actions(ctx: PoiseCtx<'_>) -> AppResult {
    ...
    ctx.reply("Non-read user actions are now blocked. Use `/unblock` to unblock all actions.")
        .await?;
    Ok(())
}
...
pub async fn block_moderator_actions(ctx: PoiseCtx<'_>) -> AppResult {
    ...
    ctx.reply("Non-read moderator actions are now blocked. Use `/unblock` to unblock all actions.")
        .await?;
    Ok(())
}
...
pub async fn block_all(ctx: PoiseCtx<'_>) -> AppResult {
    ...
    ctx.data().block_user_actions.store(true, Relaxed);
    ctx.data().block_moderator_actions.store(true, Relaxed);
    ctx.reply("All non-read actions are now blocked. Use `/unblock` to unblock all actions.")
        .await?;
    Ok(())
}
